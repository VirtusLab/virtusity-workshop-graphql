...
const resolvers: Resolvers = {
  ...
  Human: {
    ...
    friendsConnection: ({ friends }, { firstArg, afterArg }) => {
      const first = firstArg || friends.length;
      const after = afterArg ? parseInt(fromCursor(afterArg), 10) : 0;
    ...
    starships: ({ ships }) => ships.map(getStarship),
